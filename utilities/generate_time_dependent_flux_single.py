"""
Time-dependent flux field generator for STRATOS with single VTS output

This script generates realistic Van Allen radiation belt flux fields that evolve
over time during a coronal mass ejection (CME) event. Uses a single VTS file
with time as a scalar field.
"""
...
def create_time_dependent_single_vts(
    output_dir="data/flux/time_dependent",
    time_start=-12,    # Start 12 hours before CME
    time_end=48,       # End 48 hours after CME arrival  
    time_step=3,       # 3-hour time steps
    nx=30, ny=30, nz=30  # Grid resolution
):
    """
    Create a single VTS file with time-dependent flux data as multiple scalar fields.
    
    Args:
        output_dir: Directory to store VTS file
    ...
    print(f"Creating time-dependent Van Allen belt flux dataset")
    ...
    # Create points (same for all time steps)
    ...
    for t_idx, current_time in enumerate(time_points):
        ...
        # Add flux data to grid as a named array
        flux_array = numpy_to_vtk(np.array(flux_values))
        
        # Use the first time step as the primary scalars
        if t_idx == 0:
            flux_array.SetName("electron_flux")
            sgrid.GetPointData().SetScalars(flux_array)
        else:
            # Additional time steps as named arrays
            flux_array.SetName(f"electron_flux_t{current_time:+.0f}h")
            sgrid.GetPointData().AddArray(flux_array)
    ...
    # Add time values as a separate array for reference
    time_array = numpy_to_vtk(np.tile(time_points, (total_points, 1)).flatten())
    time_array.SetName("time_values")
    sgrid.GetFieldData().AddArray(time_array)
    
    # Write single VTS file
    filename = f"{output_dir}/time_dependent_flux.vts"
    print(f"\nWriting VTS file: {filename}")
    ...
    print("\n" + "=" * 60)
    print("Time-dependent flux VTS dataset generated successfully!")
...
def create_time_series_metadata(output_dir, time_points):
    ...
    metadata = {
        'description': 'Time-dependent Van Allen belt flux with CME effects (single VTS)',
        'file_type': 'vtk_structured_grid',
...
if __name__ == "__main__":
    # Generate time-dependent flux with single VTS format
    create_time_dependent_single_vts(
        output_dir="data/flux/time_dependent",
        time_start=-12,  # 12 hours before storm
        time_end=48,     # 48 hours after storm arrival
        time_step=6,     # 6-hour time steps (11 total steps for manageable size)
        nx=25, ny=25, nz=25  # Smaller resolution for faster loading
    )
    
    print("\n" + "=" * 60)
    print("Time-dependent Van Allen belt simulation complete!")
    print("Load the VTS file in STRATOS to visualize storm evolution")
    print("=" * 60)